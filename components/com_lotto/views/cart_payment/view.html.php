            ...
            if ($user->id != 0) {
                ...
                if ($deducted_amount >= 0) {
                 ...
                 $session->clear("cart"); // clear cart
                    ...
                    $this->text = "Недостаточно средств на счету пожалуйста пополните баланс здесь";
                    $this->amount = $amount;
    ...
    public function insert_tickets($lotto_tickets, $user_id){
        ...
        for($i=1; $i <=$lotto_tickets['ticket_tiraje'];$i++) {
            foreach ($lotto_tickets['tickets_numbers'] as $ky=>$ticket_num) {
                ...
                $query = "SELECT * FROM #__lotto_tiraje WHERE Tiraje_number='".$lotto_tickets['ticket_tiraje_number']."'";
                ...
                $profile->tickets_tiraj_numbers = $rows[0]->id;
